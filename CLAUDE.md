...
UI に触る変更は、自動テストで守れない部分がある。**実際に動かすのが唯一の検証手段になる。**

**動作の検証は実機で行う。** このプロジェクトでは端末依存の挙動に何度か当たっている
（IME の入力、`uiautomator dump` の出力範囲、`logcat` の安定性）。

**スクリーンショットはエミュレータでよい。** 見た目を伝えるのが目的なので、
実機が手元に無くても止まらない。`emulator -list-avds` で使える AVD を確認する。
...
- **UI を変えたらスクリーンショットを撮り、ステップ 6 でユーザーに渡す。**
  スクラッチパッドに置き、リポジトリには入れない。変更前と変更後、
  分岐があるなら両方を撮る。**実機でもエミュレータでもよい**
